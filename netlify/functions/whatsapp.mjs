...
async function markReadAndTyping(messageId) {
  try {
    ...
    // No bloqueamos la respuesta si falla el indicador de lectura/escritura.
...
async function getConversation(store, waId) {
  ...
  const ttlHours = Number(env("CONVERSATION_TTL_HOURS", "24")) || 24;
  const expired = Date.now() - Number(saved.updatedAt || 0) > ttlHours * 3600_000;

  return expired ? { history: [], updatedAt: 0 } : saved;
}
...
async function processTextMessage(message) {
  ...
  if (!waId || !messageId || !userText) return;

  const dedupe = getStore("velturio-whatsapp-processed");
  ...
  try {
    ...
    const conversations = getStore({ name: "velturio-whatsapp-conversations", consistency: "strong" });
    ...
    const conversation = await getConversation(conversations, waId);
    const reply = await askVeli(conversation.history, userText);
    ...
    try { await conversations.set(
      ...
      // La respuesta ya salió: no volver a enviarla por un fallo del historial.
      console.error("No se pudo guardar el historial:", error.message);
    }
  } catch (error) {
    console.error("Error procesando mensaje:", error);

    // Permite que Meta reintente el webhook si el procesamiento falló.
    try { await dedupe.delete(messageId); } catch {}
...
export default async (request) => {
  ...
  // 1) Verificación inicial del webhook de Meta.
  ...
  if (request.method === "POST") {
    if (env("BOT_ENABLED", "true").toLowerCase() !== "true") {
      return json({ ok: true, bot: "disabled" });
    }
      ...
      for (const change of entry?.changes ?? []) {
        ...
        for (const message of value?.messages ?? []) {
          if (message?.type === "text") {
            jobs.push(sendText(processTextMessage(message)));
            ...
            jobs.push(
              (async () => {
                ...
                const claim = await dedupe.set(
                  message.id,
                  JSON.stringify({ startedAt: Date.now(), unsupported: true }),
                  { onlyIfNew: true }
                );
                if (claim.modified) {
                  try { await sendText(
                    ...
                    await dedupe.delete(message.id);
                    throw error;
                  }
                }
              })()
    ...
    if (results.some((result) => result.status === "rejected")) {
      return json({ ok: false, error: "Processing failed; retry later" }, 503);
    }
    return json({ ok: true });
  }
